...
export const RANGE_LABELS: Record<RangeType, string> = {
  day: "Day", week: "Week", month: "Month", year: "Year", years: "Multi-year", all: "All time",
};

// How many years the "Multi-year" view spans, ending at the anchor year.
const YEARS_SPAN = 5;
...
/** Resolve a granularity + anchor date into a [start, end) string range and a label. */
...
export function shiftAnchor(type: RangeType, anchor: string, dir: 1 | -1): string {
  const a = parseISO(anchor);
  switch (type) {
    case "day": return iso(addDays(a, dir));
    case "week": return iso(addWeeks(a, dir));
    case "month": return iso(addMonths(a, dir));
    case "year": return iso(addYears(a, dir));
    case "years": return iso(addYears(a, dir * YEARS_SPAN));
    default: return anchor;
  }
}

/** "all" and the broad views can't navigate; used to disable prev/next. */
